...
static void handle_authentication(lv_event_t* event) {
  ...
  if (strcmp(password_input, AUTH_SCREEN_PASSWORD) != 0) {
    lv_msg_send(BAD_AUTH_ATTEMPT_MSG, NULL);
  } else {
    lv_msg_send(SUCCESSFUL_AUTH_MSG, NULL);
    animate_screen_change(settings_screen, LV_SCR_LOAD_ANIM_MOVE_LEFT);
  }
}

static void handle_settings_screen_gestures(lv_event_t* event) {
  lv_dir_t direction = lv_indev_get_gesture_dir(lv_indev_get_act());
  if (direction != LV_DIR_RIGHT) return;
  animate_screen_change(main_screen, LV_SCR_LOAD_ANIM_MOVE_RIGHT);
}
...
void start_screeen_management(void) {
  ...
  lv_obj_add_event_cb(settings_screen, handle_settings_screen_gestures, LV_EVENT_GESTURE, NULL);
}